...
const Prizes = () => {
  ...
  return (
    <section id="prizes" className="py-24 relative bg-secondary/30" ref={ref}>
      <div className="container mx-auto px-4">
        ...
        >
          ...
          <p className="mt-4 text-lg font-mono text-muted-foreground">
            TOTAL PRIZES WORTH{" "}
            <span className="text-primary text-glow text-2xl">₹5,00,000+</span>
          </p>
        </motion.div>
        ...
        <div className="grid md:grid-cols-3 gap-6 mb-16">
          {prizes.map((prize, index) => (
            <motion.div
              key={prize.position}
              className={`relative p-6 border border-border bg-card/50 backdrop-blur-sm ${
                index === 0 ? "md:-mt-8" : ""
              }`}
            ...
            >
              {/* Position badge */}
              <div
                className={`absolute -top-4 left-1/2 -translate-x-1/2 px-4 py-1 bg-gradient-to-r ${prize.color} text-background font-mono font-bold text-sm`}
              >
                {prize.position} PLACE
              </div>

              <div className="pt-6 text-center">
                <prize.icon className="w-16 h-16 text-primary mx-auto mb-4" />
                <p className="text-4xl md:text-5xl font-display font-bold text-foreground mb-4">
                  {prize.prize}
                </p>
        ...
        >
          <h3 className="text-xl font-display font-bold text-center mb-6">
            <Gift className="inline w-5 h-5 text-primary mr-2" />
            TRACK PRIZES
          </h3>
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
            {trackPrizes.map((track) => (
              <div
                key={track.track}
                className="p-4 border border-border bg-card/30 text-center hover:border-primary transition-colors"
              >
                <p className="text-xs font-mono text-primary mb-2">
                  {track.track}
                </p>
