...
import { Sparkles, Satellite, Globe, Waypoints, MapPin, Flame, Rocket, Brain, Radio, Flag, Award, Utensils, Gift, Ticket } from "lucide-react";
...
const PhaseHeaderCards = () => {
  return <div className="grid md:grid-cols-2 gap-6 mb-16">
      {/* Phase I - Online */}
      <motion.div className="relative p-6 rounded-2xl border border-dashed border-foreground/20 bg-background/50 backdrop-blur-sm" initial={{
      opacity: 0,
      x: -30
    }} whileInView={{
      opacity: 1,
      ...
      {/* Phase II - Offline */}
      <motion.div className="relative p-6 rounded-2xl border border-foreground/30 bg-foreground/5 backdrop-blur-sm" initial={{
      opacity: 0,
      x: 30
    }} whileInView={{
      opacity: 1,
...
const SelectionGateDivider = () => {
  return <motion.div className="relative py-8 my-4 z-20" initial={{
    opacity: 0
  }} whileInView={{
    opacity: 1
  }} viewport={{
    ...
    duration: 0.8
      ...
      <div className="relative flex justify-center z-10">
        <motion.div className="px-6 py-3 bg-background border border-foreground/30 rounded-full flex items-center gap-3" animate={{
        boxShadow: ['0 0 20px hsl(var(--foreground) / 0.1)', '0 0 40px hsl(var(--foreground) / 0.2)', '0 0 20px hsl(var(--foreground) / 0.1)']
      }} transition={{
        duration: 2,
        repeat: Infinity
      }}>
...
const DynamicJourney = () => {
  ...
  return <section ref={containerRef} id="journey" className="relative py-32 overflow-hidden bg-card/30">
      <div className="container mx-auto px-4 relative z-10">
        ...
        duration: 0.8
          ...
          <motion.div className="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-foreground/10 bg-foreground/5 mb-6" whileHover={{
          scale: 1.02
        }}>
        ...
        <div ref={timelineRef} className="relative max-w-4xl mx-auto">
          ...
          <div className="absolute left-6 md:left-1/2 top-0 bottom-0 w-px bg-border md:-translate-x-1/2">
            <motion.div className="w-full bg-foreground" style={{
            height: lineHeight
          }} />
          </div>
          ...
          return <TimelineEvent key={phase.id} phase={phase} isEven={isEven} index={currentIndex} />;
        })}

          {/* Transition Phase - Selection Gate (LEFT side to continue alternation) */}
          {transitionPhase && <TimelineEvent key={transitionPhase.id} phase={transitionPhase} isEven={false} index={alternationIndex++} isTransitionPhase />}
...
const TimelineEvent = ({
  ...
  return <motion.div className="relative flex items-start md:items-center gap-4 md:gap-8 mb-12 md:mb-16" initial={{
    opacity: 0,
    y: 30
  }} whileInView={{
    opacity: 1,
    ...
    delay: index * 0.03
      ...
      <motion.div className="relative z-10 flex-shrink-0" whileHover={{
      scale: 1.1
    }}>
        <motion.div className={`w-12 h-12 md:w-14 md:h-14 rounded-full flex items-center justify-center ${isTransition ? 'bg-foreground/20 border-2 border-foreground' : isOffline ? 'bg-foreground' : 'border-2 border-dashed border-foreground/50 bg-background'}`}>
          <phase.icon className={`w-5 h-5 md:w-6 md:h-6 ${isTransition ? 'text-foreground' : isOffline ? 'text-background' : 'text-foreground/70'}`} />
        </motion.div>
      </motion.div>
...
const EventCard = ({
  ...
  const isTransition = phase.round === 'transition';
  const isOffline = phase.round === 'offline';
  return <motion.div className={`p-4 md:p-6 rounded-2xl ${isTransition ? 'border-2 border-foreground/40 bg-foreground/10' : isOffline ? 'border border-foreground/30 bg-foreground/5' : 'border border-dashed border-foreground/20 bg-background/50'} ${alignRight ? 'text-right' : 'text-left'}`} whileHover={{
    scale: 1.02,
    borderColor: isTransition ? 'hsl(var(--foreground) / 0.6)' : 'hsl(var(--foreground) / 0.3)'
      ...
      <div className={`flex items-center gap-2 ${alignRight ? 'justify-end' : 'justify-start'}`}>
        ...
        {phase.badge && <Badge variant="outline" className={`text-[10px] tracking-wider ${isOffline ? 'border-foreground/40 text-foreground/70' : 'border-foreground/20 text-foreground/50'}`}>
            {phase.badge}
          </Badge>}
      </div>
